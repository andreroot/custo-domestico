create or replace table devsamelo.dev_custo.custo_comparativo as (
with ano_atual as(

    select 
        dt_pagto_bq data_base_bq,
        extract(month from dt_pagto_bq) mes_base_ordem,
        descri____o descricao,
        --lan__amento lancamento,
        sum(valor_convertido_saida__R__) valor_2021,


    from devsamelo.dev_custo.custo_2021 atl
    group by dt_pagto_bq, mes_base_ordem, descri____o--, lan__amento
...
left join (
    select 
        dt_pagto_bq data_base_bq,
        extract(month from dt_pagto_bq) mes_base_ordem,
        descri____o descricao_2020,
        --lan__amento lancamento,
        sum(valor_convertido_saida__R__) valor_2020,
    from devsamelo.dev_custo.custo_2020 old
    group by dt_pagto_bq, mes_base_ordem, descri____o--, lan__amento
) old_20 
...
left join (
    ...
    from devsamelo.dev_custo.custo_2019 old
